...
function getFromArrayNumberPeriods($array, $year, $initialYear){
    return $array[getNumberPeriods(count($array)*2, $year, $initialYear)];
}

/*
 * Hay que hacer la distinción porque PHP no calcula bien el módulo de números negativos
 * El ultimo modulo del else es para controlar los años que distan del año inicial un multiplo del tamaño del ciclo
*/

function getNumberPeriods($yearsCicle, $year, $initialYear){
    if($year >= $initialYear) {
