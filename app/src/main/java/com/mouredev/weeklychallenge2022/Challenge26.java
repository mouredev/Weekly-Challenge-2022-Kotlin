public class Challenge26 {
    ...
    private static String buildGeometry(Geometry geometry, int sideLength) {
        ...
        for (int i = 1; i <= sideLength; i++) {
            if (geometry.equals(Geometry.SQUARE)) {
                if(i==1 || i==sideLength){
                    res.append(new String(new char[sideLength]).replace('\0', '*')).append('\n');
                }else{
                    res.append('*').append(new String(new char[sideLength-2]).replace('\0', ' ')).append('*').append('\n');
                }
                ...
                if(i==1){
                    ...
                    line.setCharAt(sideLengthAdapted/2, '*');
                    ...
                    StringBuilder line = new StringBuilder(String.valueOf(new char[sideLengthAdapted]).replace('\0', ' '));
                    line.setCharAt(sideLengthAdapted/2 + (i-1), '*');
                    line.setCharAt(sideLengthAdapted/2 - (i-1), '*');
