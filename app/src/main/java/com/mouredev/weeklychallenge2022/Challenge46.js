...
module.exports.func = function robotPosition(input) {
  let x = 0;
  let y = 0;
  input.forEach((distance, index) => {
    if (index % 2 != 0) x += direction(distance, index + 1);
    if (index % 2 == 0) y += direction(distance, index);
  });
  return { x: x, y: y };
};

const direction = (distance, index) =>
  index % 4 == 0 ? distance : distance * -1;
