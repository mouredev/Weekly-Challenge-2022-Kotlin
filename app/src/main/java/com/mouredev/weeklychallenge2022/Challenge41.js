...
function ohm({V, R, I}: {V?: number, R?: number, I?: number}) {
  const paramsCounter = [V, R, I].filter((x) => x ).length;
  if (paramsCounter < 2 || R === 0 || I === 0) {
    return "Invalid values"
  }
  
  return I && R ? Math.round((I * R) * 100) / 100 :
				 V && R	? Math.round((V / R) * 100) / 100 :
