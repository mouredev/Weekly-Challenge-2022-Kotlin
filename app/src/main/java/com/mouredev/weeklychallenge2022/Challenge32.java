...
public class SegundoMayor {
    /**
     * Implementación de menor complejidad (complejidad lineal O(n)) solo se recorre una vez la lista
     * y se dejan los 2 mayores encontrados. En caso de encontrar un mayor pasara el mayor actual
     * detectado al segundo lugar. 
     * <p>
     * Si ordenaramos el array y lo recorrieramos hasta detectar un cambio tambíen lo encontrariamos
     * pero la complejidad dejaría de ser lineal. El método de ordenación collections.sort complejjidad O(n log (n))
     *
     */
    public static void main(String[] args) {
        Double[] numbers = {1.5, 2.0, 11.0, 3.0, 5.0, 11.0, 9.0, 3.0, 3.0009, 2.03, 2.05};
        ...
        listNumbers.forEach(
                (final Double number) -> {
                    ...
                    if(number!=primeroMayor && number!=segundoMayor){
                        if (number != primeroMayor && number > primeroMayor) {
                            listMayores.set(0, number);
                            listMayores.set(1, primeroMayor);
                        } else if (number != segundoMayor && number > segundoMayor) {
